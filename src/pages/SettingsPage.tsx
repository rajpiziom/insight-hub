...
import { Settings, User, Palette, LayoutGrid, LogOut } from 'lucide-react';
...
const allTopics = ['markets', 'macro', 'technology', 'geopolitics', 'business', 'ai', 'regulation', 'china', 'fed', 'energy', 'policy'];
      ...
      <div className="space-y-8">
        <motion.section initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="bg-card border border-border rounded-xl p-6">
          ...
          <div className="flex flex-wrap gap-2">
            {allTopics.map(topic => (
              <button key={topic} onClick={() => toggleFavorite(topic)}
                className={cn('px-3 py-1.5 rounded-lg text-xs font-medium transition-all border',
                  favoriteTopics.includes(topic) ? 'bg-primary/10 text-primary border-primary/30' : 'bg-muted text-muted-foreground border-transparent hover:border-border')}>
                {topic}
              </button>
            ))}
          </div>
          ...
          <div className="space-y-5">
            ...
            <div>
              ...
              <select value={sortMode} onChange={(e) => setSortMode(e.target.value)}
                ...
                <option value="date">Date</option><option value="relevance">Relevance</option><option value="source">Source</option>
              </select>
            </div>
            <div>
